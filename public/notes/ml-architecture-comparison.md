...
- first thing I noted is the similarity between these to architectures
- the most glaring differences were the and switching the trainable parameters on and off throughout training
- creating the actual model is pretty much identical, when using Sequential, only slightly different syntax
- after some research, when creating custom models and training loops, that's where tensorflow's gradient tape really would shine, but I did not bump into the tape in this project
- pytorch, I did note, was easier to use and understand just looking back at the code
- one huge difference, as of May 2026, was that tensorflow code does not run on NVIDIA GPU (CUDA) on Windows, which was suprising to me, although I've overheard the accessability will return soon to Windows
- as a workaround I used WSL, which was interesting to learn about
- another big difference was stark contrast in accurcies I got on the same pretrained models with the same hypers
- the pytorch implementation (the side that I optimized for hypers) received close to 90% acc, while the same setup with tensorflow initially was consistently in the lower 80 even high 70s
...
- one surprising result was how well (again, based on the statis best config from the scratch cnn section) the EfficientNet performed

## Scikit-Learn
- for the sklearn section, I wanted to experiment with the different popular classifier sklearn had
- the way I could implement this was to take the final feature vector and use that as the input for the classifiers
- since the output of the pre-trained models had such rich final feature vectors, the classifiers had no problem training those data
- surpisingly, the classifiers performed as well as fine tuning the models themselves
- I believe this is because the pre-trained models are just that good at providing unique rich features for every input
- I added some more visuals to the sklearn section just to fill up space, a good place to add the pretrained models PCA and I just through in a confusion matrix to show that I know it exists (although it is interesting to see that mountains and glaciers get mistaken for each other most commonly)
...
- most of the literature and courses I've taken seem to defaul to Adam, but AdamW got me curious
- I read some posts and watched some education videos on the difference between the two optimizers and came to the conclusion that the difference is small, but AdamW is the mathematically correct optimizer, corrected from a mistake the original Adam optimizer had
...
- I decided to stick to one set of hypers for all fine-tuning, I realize this makes the results not accurate, since every model likely had better hypers that would optimizet their accuracies, however, that would be hundereds of runs and the point of the project would shift to just brute force grid search computing and I have other projects to get to
- all that to say that resnet, although the winner on paper, could have not been under certain hyper configs
- also to say that fine-tuned resnet model would have likely had an even better final accuracy if spent the time to optimize their hypers as well
...
- this is important to not because if you search up 'staged fine-tuning' on the web, a paper from 2018 by Sebastian Ruder and Jeramy Howard detailing ULMFiT (Universal Language Model Fine-Tuning), in which they describe unfreezing a layer after each epoch
...
- I chose to focus on the ones that I was taught were the most impactful, i.e. learning rate, dropout, etc.
